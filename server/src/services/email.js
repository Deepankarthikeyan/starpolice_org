...
export function formatDeliveryError(message) {
  const text = typeof message === "string" ? message.trim() : "";
  if (!text) return "Email delivery failed.";

  if (/only send testing emails/i.test(text)) {
    return "Resend is in test mode (onboarding@resend.dev). Verify your domain at resend.com and set EMAIL_FROM to an address on that domain, e.g. Star Police Academy <noreply@starpoliceacademy.in>.";
  }

  if (/domain is not verified/i.test(text)) {
    return "The sender domain is not verified in Resend. Add and verify your domain, then update EMAIL_FROM on the API server.";
  }

  return text;
}

export function buildEmailStatusMessage(emailResult, {
  successMessage,
  notConfiguredMessage,
  failureMessage,
}) {
  if (emailResult?.delivered) return successMessage;
  if (emailResult?.devMode) return notConfiguredMessage;
  if (emailResult?.deliveryError) {
    return `${failureMessage}: ${formatDeliveryError(emailResult.deliveryError)}`;
  }
...
function logDevFallback(payload, reason = "delivery failed") {
  console.log(`[email:fallback] ${reason} — logged to console:`);
...
export async function sendPasswordEmail({
  ...
  if (!isEmailConfigured()) {
    logDevFallback({ to, subject, setupUrl, otp: isOtpOnly ? otp : undefined }, "email not configured");
  ...
  try {
    ...
    const deliveryError = error.message || "Email delivery failed.";
    console.error("[email] Delivery failed:", deliveryError);
    logDevFallback(
      { to, subject, setupUrl, otp: isOtpOnly ? otp : undefined, error: deliveryError },
      "delivery failed",
    );
    return {
      delivered: false,
      ...
      deliveryError: formatDeliveryError(deliveryError),
    };
  }
}

...
import { buildEmailStatusMessage } from "../services/email.js";
  ...
  async (req, res) => {
    try {
      ...
      res.status(201).json({
        ...
        setupUrl: !emailResult?.delivered ? emailResult?.setupUrl : undefined,
        deliveryError: emailResult?.deliveryError,
        message: buildEmailStatusMessage(emailResult, {
          successMessage:
          ...
          failureMessage: "Account created but email could not be sent",
        }),
      });
    } catch (error) {
    ...
    try {
      ...
      res.json({
        message: buildEmailStatusMessage(emailResult, {
          ...
          failureMessage: "Invite email could not be sent",
        }),
        inviteSent: true,
        delivered: emailResult?.delivered ?? false,
        devMode: emailResult?.devMode ?? false,

...
import { isEmailConfigured, getEmailProvider, isResendSandboxFrom, getFromAddress } from "./services/email.js";
...
app.get("/api/health", (_req, res) => {
  ...
  res.json({
    ...
    email: {
      ...
      from: isEmailConfigured() ? getFromAddress() : undefined,
      sandbox: isEmailConfigured() && getEmailProvider() === "resend" && isResendSandboxFrom(),
    },
...
async function start() {
  ...
  if (isEmailConfigured()) {
    ...
    console.log(`Email delivery enabled (${provider})`);
    if (provider === "resend" && isResendSandboxFrom()) {
      console.warn(
        "Resend is using onboarding@resend.dev — emails only reach your Resend account email until you verify a domain and set EMAIL_FROM.",
      );
    }
  } else {
    console.warn(

...
app.prepare().then(() => {
  ...
  serveDirectory(server, "/panel", "public/panel");
  ...
  server.get(/^\/(admin|staff|student)(?:\/.*)?$/, (_request, response, nextMiddleware) => {
    const panelIndex = path.join(rootDir, "public", "panel", "index.html");
    if (require("fs").existsSync(panelIndex)) {
      response.sendFile(panelIndex);
      return;
    }
    nextMiddleware();
  });
